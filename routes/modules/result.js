    ...
    ShortUrl.findOne({ url: longURL })
      .then((result) => {
        ...
        if (result) {
          ...
          const recordMsg = `您輸入的網址曾產生出這個短網址：<p class="fw-bold" id="shortUrl">${shortUrl}</p>如果要重新輸入網址，請使用瀏覽器的重新整理按鈕。`;
          res.render("result", { result: recordMsg, copy: copyButton });
        } else {
          const shortUrl = newShortUrl;
          const successMsg = `短網址產生完畢：<p class="fw-bold" id="shortUrl">${shortUrl}</p>如果要重新輸入網址，請使用瀏覽器的重新整理按鈕。`;
